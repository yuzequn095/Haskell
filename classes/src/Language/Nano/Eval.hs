...
evalE env (EBin o e1 e2) = do o1 <- evalE env e1
			      o2 <- evalE env e2
			      return (evalOp o o1 o2)
evalE env (EIf c t e)    = 
			  if bool == True then evalE env t
			  else evalE env e
			  where 
				(VBool bool) = eval env c
evalE env (ELet x e1 e2) = evalE ((x, eval env e1):env) e2
--evalE env (EApp e1 e2)   = case (evalE env e1) of
--				Right (VClos oldEnv x body) -> evalE ((x, eval env e2):oldEnv) body
--				Right (VPrim prim) -> Right (prim(eval env e2))
--			        _ -> throw (Error "type error")
evalE env (EApp e1 e2)   = case (evalE env e1) of
			      Right (VClos oldEnv x body) -> evalE (((x,eval env e2) : oldEnv) ++ env) body
			      Right (VPrim prim) -> Right (prim (eval env e2))
			      _ -> throw (Error "type error")
