...
exprToString (CubeSum e1 e2 e3)      = exprToString e1 ++ "*" ++ exprToString e2 ++ "*" ++ exprToString e3 ++ "+" ++ exprToString e1 ++ "*" ++ exprToString e2 ++ "*" ++ exprToString e3
-- exprToString (SquareSum e1 e2)    = exprToString e1 ++ "*" ++ exprToString e1 ++ "+" ++ exprToString e2 ++ "*" ++ exprToString e2
exprToString (Pick e1 e2 e3)      = "(" ++ exprToString e1 ++ ">" ++ "0?" ++ exprToString e2 ++ ":" ++ exprToString e3 ++ ")"
--------------------------------------------------------------------------------
...
eval x y e = case e of 
        ...
        Thresh e1 e2 e3 e4 -> if ((eval x y e1) < (eval x y e2))
				then (eval x y e3)
			      else 
				      (eval x y e4)
	CubeSum e1 e2 e3 -> (eval x y e1) * (eval x y e2) * (eval x y e3) + (eval x y e1) * (eval x y e2) * (eval x y e3) 
--        SquareSum e1 e2 -> (eval x y e1) * (eval x y e1) + (eval x y e2) * (eval x y e2) 
        Pick e1 e2 e3 -> if ((eval x y e1) > 0)
                               then (eval x y e2)
                         else
                               (eval x y e3)
...
build d       
  | p == 0    = Sine (build (d-1))
  | p == 1    = Cosine (build (d-1))
  | p == 2    = Average (build (d-1)) (build (d-1))
  | p == 3    = Times (build (d-1)) (build (d-1))
  | p == 4    = Thresh (build (d-1)) (build (d-1)) (build (d-1)) (build (d-1))
  | p == 5    = CubeSum (build (d-1)) (build (d-1)) (build (d-1))
--  | p == 4    =  SquareSum (build (d-1)) (build (d-1))
  | otherwise = Pick (build (d-1)) (build (d-1)) (build (d-1))
